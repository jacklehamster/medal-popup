...
Newgrounds.io.model = {
  checkStrictValue: function(
      classname, property, value, type, model, array_type, array_model) {
    ...
    if (value.constructor === Array && (array_type || array_model)) {
      for (var i = 0; i < value.length; i++) {
        this.checkStrictValue(
            classname, property, value[i], array_type, array_model, null, null);
      }
      return true;
    }
    if (classname)
      throw new Error(
          'Illegal \'' + property + '\' value set in model ' + classname);
    return false;
  },
};
Newgrounds.io.events.OutputEvent = function(type, call, data) {
  ...
  this.call = call;
  this.data = data;
  this.success = data && typeof (data['success'] != 'undefined') ?
      (data.success ? true : false) :
      false;
  this.preventDefault = false;
};
Newgrounds.io.events.OutputEvent.prototype.constructor =
    Newgrounds.io.events.OutputEvent;
...
Newgrounds.io.events.SessionEvent.prototype.constructor =
    Newgrounds.io.events.SessionEvent;
...
Newgrounds.io.events.EventDispatcher.prototype.constructor =
    Newgrounds.io.events.EventDispatcher;
Newgrounds.io.core = function(app_id, aes_key) {
  ...
  if (!app_id)
    throw new Error(
        'Missing required \'app_id\' in Newgrounds.io.core constructor');
  if (typeof app_id != 'string')
    throw new Error(
        '\'app_id\' must be a string value in Newgrounds.io.core constructor');
  _app_id = app_id;
  if (aes_key)
    _aes_key = CryptoJS.enc.Base64.parse(aes_key);
  else
    console.warn(
        'You did not set an encryption key. Some calls may not work without this.');
  var _session_storage_key =
      'Newgrounds-io-app_session-' + _app_id.split(':').join('-');
  function checkLocalStorage() {
    if (typeof localStorage != 'undefined' && localStorage &&
        localStorage.getItem.constructor == Function)
      return true;
    console.warn(
        'localStorage unavailable. Are you running from a web server?');
    return false;
  }
  ...
  this.addEventListener('App.checkSession', (e) => {
    if (e.success) {
  ...
  this._encryptCall = function(call_model) {
    if (!call_model ||
        !call_model.constructor == Newgrounds.io.model.call_model)
      throw new Error('Attempted to encrypt a non \'call\' object');
    var iv = CryptoJS.lib.WordArray.random(16);
    var encrypted = CryptoJS.AES.encrypt(
        JSON.stringify(call_model.toObject()), _aes_key, {iv: iv});
...
Newgrounds.io.core.prototype = {
  ...
  _event_listeners: {},
  addEventListener:
      Newgrounds.io.events.EventDispatcher.prototype.addEventListener,
  removeEventListener:
      Newgrounds.io.events.EventDispatcher.prototype.removeEventListener,
  removeAllEventListeners:
      Newgrounds.io.events.EventDispatcher.prototype.removeAllEventListeners,
  ...
  getSessionLoader: function() {
    if (this._session_loader == null)
      this._session_loader = new Newgrounds.io.SessionLoader(this);
  ...
  requestLogin: function(
      on_logged_in, on_login_failed, on_login_cancelled, context) {
    if (!on_logged_in || on_logged_in.constructor !== Function)
  ...
  cancelLoginRequest: function() {
    event =
        new Newgrounds.io.events.OutputEvent('cancelLoginRequest', null, null);
  ...
  _doCallback: function(call_model, callback, o_return, context) {
    ...
    var o_error = {
      success: false,
      error: {code: 0, message: 'Unexpected Server Response'}
    };
    if (typeof o_return == 'undefined') o_return = null;
    if (call_model.constructor === Array && callback &&
        callback.constructor === Array) {
      for (i = 0; i < call_model.length; i++) {
        x_return = !o_return || typeof o_return[i] == 'undefined' ? o_error :
                                                                    o_return[i];
    ...
    if (o_return) {
      if (typeof o_return['data'] != 'undefined') {
        ...
        console.warn(
            'Received empty data from \'' + call_model.component + '\'.');
        o_data = null;
      }
    } else {
      o_data = o_error;
    ...
    if (o_data.constructor === Array) {
      for (i = 0; i < o_data.length; i++) {
        event = new Newgrounds.io.events.OutputEvent(
            call_model.component, call_model[i], o_data[i]);
        this.dispatchEvent(event);
      }
    } else {
      event = new Newgrounds.io.events.OutputEvent(
          call_model.component, call_model, o_data);
      this.dispatchEvent(event);
    }
  ...
  _formatResults: function(component, result_object) {
    ...
    for (i in formats) {
      if (typeof result_object[i] == 'undefined' &&
          result_object.success !== false) {
        console.warn(
            'Newgrounds.io server failed to return expected \'' + i +
            '\' in \'' + component + '\' data.');
        continue;
      }
      if (typeof formats[i]['array'] != 'undefined') {
        ...
        if (typeof Newgrounds.io.model[model_name] == 'undefined') {
          console.warn(
              'Received unsupported model \'' + model_name + '\' from \'' +
              component + '\'.');
          continue;
        }
        if (result_object[i].constructor !== Array) {
          console.warn(
              'Expected array<' + model_name + '> value for \'' + i +
                  '\' in \'' + component + '\' data, got ' +
        ...
        result_object[i] = models;
          ...
          typeof formats[i]['object'] != 'undefined' && result_object[i]) {
  ...
  _doCall: function(call_model, callback, context) {
    ...
    function checkRedirect(model) {
      var validator =
          Newgrounds.io.call_validators.getValidator(model.component);
      if (validator.hasOwnProperty('redirect') && validator.redirect) {
        var parameters = model.parameters;
        if (!parameters || !parameters.hasOwnProperty('redirect') ||
            parameters.redirect) {
          return true;
        }
      }
    ...
    if (call_model.constructor === Array) {
      ...
      for (i = 0; i < call_model.length; i++) {
        if (checkRedirect(call_model[i])) {
          throw new Error(
              'Loader components can not be called in an array without a redirect=false parameter.');
        }
    ...
    if (is_redirect) {
      ...
      var jhax = typeof Array.prototype.toJSON != 'undefined' ?
  ...
  _doValidateCall: function(component, parameters) {
    ...
    if (!validator)
      throw new Error('\'' + component + '\' is not a valid server component.');
    if (validator.require_session && !this.session_id)
      throw new Error('\'' + component + '\' requires a session id');
    ...
    for (param in validator.params) {
      ...
      param_value = parameters && typeof parameters[param] != 'undefined' ?
          parameters[param] :
          null;
      ...
      if (param_value === null) {
        if (rules.required)
          throw new Error(
              'Missing required parameter for \'' + component + '\': ' + param);
        continue;
      }
      if (rules.extract_from &&
          param_value.constructor ===
              Newgrounds.io.model[rules.extract_from.object]) {
        param_value = param_value[rules.extract_from.property];
      }
      if (!Newgrounds.io.model.checkStrictValue(
              null, param, param_value, rules.type, null, null, null))
        throw new Error(
            'Illegal value for \'' + param + '\' parameter of \'' + component +
            '\': ' + param_value);
    }
  },
  _validateCall: function(call_model) {
    ...
    if (call_model.constructor === Array) {
      ...
      throw new Error(
          'Unexpected \'call_model\' value. Expected Newgrounds.io.model.call instance.');
    }
    ...
    var validator =
        Newgrounds.io.call_validators.getValidator(call_model.component);
...
Newgrounds.io.urlHelper = function() {
  ...
  this.getRequestQueryParam = function(param_name, default_value) {
    ...
    return typeof requestParams[param_name] == 'undefined' ?
        default_value :
        requestParams[param_name];
  };
};
Newgrounds.io.model.call = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'component', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'component', __vv__, String, null, null, null);
      _component = __vv__;
    },
  });
  var _echo;
  ...
  Object.defineProperty(this, 'parameters', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'parameters', __vv__, Object, null, Object, null);
      _parameters = __vv__;
    },
  });
  var _secure;
  Object.defineProperty(this, 'secure', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'secure', __vv__, String, null, null, null);
      _secure = __vv__;
    },
  });
...
Newgrounds.io.model.debug = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'exec_time', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'exec_time', __vv__, String, null, null, null);
      _exec_time = __vv__;
    },
  });
  var _input;
  Object.defineProperty(this, 'input', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'input', __vv__, null, 'input', null, null);
      _input = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.error = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'code', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'code', __vv__, Number, null, null, null);
      _code = __vv__;
    },
  });
  var _message;
  Object.defineProperty(this, 'message', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'message', __vv__, String, null, null, null);
      _message = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.input = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'app_id', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'app_id', __vv__, String, null, null, null);
      _app_id = __vv__;
    },
  });
  var _call;
  Object.defineProperty(this, 'call', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'call', __vv__, null, 'call', null, 'call');
      _call = __vv__;
    },
  });
  var _debug;
  Object.defineProperty(this, 'debug', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'debug', __vv__, Boolean, null, null, null);
      _debug = __vv__;
    },
  });
  var _echo;
  ...
  Object.defineProperty(this, 'session_id', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'session_id', __vv__, String, null, null, null);
      _session_id = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.input.prototype.fromObject = function(object) {
  ...
  for (var i = 0; i < this.__property_names.length; i++) {
    property = object[this.__property_names[i]];
    if (this.__property_names[i] == 'call' && property)
      property = new Newgrounds.io.model.call(this.__ngio, property);
    this[this.__property_names[i]] = property;
  }
};
...
Newgrounds.io.model.medal = function(ngio, from_object) {
  ...
  this.__property_names = [
    'description', 'difficulty', 'icon', 'id', 'name', 'secret', 'unlocked',
    'value'
  ];
  ...
  Object.defineProperty(this, 'description', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'description', __vv__, String, null, null, null);
      _description = __vv__;
    },
  });
  var _difficulty;
  Object.defineProperty(this, 'difficulty', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'difficulty', __vv__, Number, null, null, null);
      _difficulty = __vv__;
    },
  });
  var _icon;
  Object.defineProperty(this, 'icon', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'icon', __vv__, String, null, null, null);
      _icon = __vv__;
    },
  });
  var _id;
  Object.defineProperty(this, 'id', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'id', __vv__, Number, null, null, null);
      _id = __vv__;
    },
  });
  var _name;
  Object.defineProperty(this, 'name', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'name', __vv__, String, null, null, null);
      _name = __vv__;
    },
  });
  var _secret;
  Object.defineProperty(this, 'secret', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'secret', __vv__, Boolean, null, null, null);
      _secret = __vv__;
    },
  });
  var _unlocked;
  Object.defineProperty(this, 'unlocked', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'unlocked', __vv__, Boolean, null, null, null);
      _unlocked = __vv__;
    },
  });
  var _value;
  Object.defineProperty(this, 'value', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'value', __vv__, Number, null, null, null);
      _value = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.output = function(ngio, from_object) {
  var _api_version, _app_id, _debug, _echo, _error, _help_url, _result,
      _success;
  this.__property_names = [
    'api_version', 'app_id', 'debug', 'echo', 'error', 'help_url', 'result',
    'success'
  ];
  ...
  Object.defineProperty(this, 'api_version', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'api_version', __vv__, String, null, null, null);
      _api_version = __vv__;
    },
  });
  var _app_id;
  ...
  Object.defineProperty(this, 'debug', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'debug', __vv__, null, 'debug', null, null);
      _debug = __vv__;
    },
  });
  var _echo;
  ...
  Object.defineProperty(this, 'error', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'error', __vv__, null, 'error', null, null);
      _error = __vv__;
    },
  });
  var _help_url;
  Object.defineProperty(this, 'help_url', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'help_url', __vv__, String, null, null, null);
      _help_url = __vv__;
    },
  });
  var _result;
  Object.defineProperty(this, 'result', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'result', __vv__, null, 'result', null, 'result');
      _result = __vv__;
    },
  });
  var _success;
  Object.defineProperty(this, 'success', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'success', __vv__, Boolean, null, null, null);
      _success = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.result = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'data', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'data', __vv__, Object, null, Object, null);
      _data = __vv__;
    },
  });
  var _echo;
...
Newgrounds.io.model.score = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'formatted_value', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'formatted_value', __vv__, String, null, null,
          null);
  ...
  Object.defineProperty(this, 'tag', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'tag', __vv__, String, null, null, null);
      _tag = __vv__;
    },
  });
  var _user;
  Object.defineProperty(this, 'user', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'user', __vv__, null, 'user', null, null);
      _user = __vv__;
    },
  });
  var _value;
...
Newgrounds.io.model.score.prototype.fromObject = function(object) {
  ...
  for (var i = 0; i < this.__property_names.length; i++) {
    property = object[this.__property_names[i]];
    if (this.__property_names[i] == 'user' && property)
      property = new Newgrounds.io.model.user(this.__ngio, property);
    this[this.__property_names[i]] = property;
  }
};
...
Newgrounds.io.model.scoreboard.prototype.postScore = function(
    score, tag, callback) {
  var board = this;
  ...
  if (this._has_ngio_user()) {
    this.__ngio.callComponent(
        'ScoreBoard.postScore', {id: this.id, value: score, tag: tag},
        function(result) {
          callback(result);
        });
  } else if (typeof callback == 'function') {
...
Newgrounds.io.model.scoreboard.prototype.constructor =
    Newgrounds.io.model.scoreboard;
Newgrounds.io.model.session = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'expired', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'expired', __vv__, Boolean, null, null, null);
      _expired = __vv__;
    },
  });
  var _id;
  Object.defineProperty(this, 'id', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'id', __vv__, String, null, null, null);
      _id = __vv__;
    },
  });
  var _passport_url;
  Object.defineProperty(this, 'passport_url', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'passport_url', __vv__, String, null, null, null);
  ...
  Object.defineProperty(this, 'remember', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'remember', __vv__, Boolean, null, null, null);
      _remember = __vv__;
    },
  });
  var _user;
...
Newgrounds.io.model.user = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'icons', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'icons', __vv__, null, 'usericons', null, null);
      _icons = __vv__;
    },
  });
  var _id;
  ...
  Object.defineProperty(this, 'supporter', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'supporter', __vv__, Boolean, null, null, null);
      _supporter = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.usericons = function(ngio, from_object) {
  ...
  Object.defineProperty(this, 'large', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'large', __vv__, String, null, null, null);
      _large = __vv__;
    },
  });
  var _medium;
  Object.defineProperty(this, 'medium', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'medium', __vv__, String, null, null, null);
      _medium = __vv__;
    },
  });
  var _small;
  Object.defineProperty(this, 'small', {
    ...
    set: function(__vv__) {
      Newgrounds.io.model.checkStrictValue(
          this.__classname, 'small', __vv__, String, null, null, null);
      _small = __vv__;
    },
  });
  if (from_object) this.fromObject(from_object);
...
Newgrounds.io.model.usericons.prototype.constructor =
    Newgrounds.io.model.usericons;
Newgrounds.io.call_validators.getValidator = function(component) {
  var c = component.split('.');
  var classname = c[0];
  var method = c[1];
  var validator = Newgrounds.io.call_validators[classname] &&
...
Newgrounds.io.call_validators.Medal = {
  getList: {
    ...
    returns: {
      medals:
          {array: {object: 'medal'}, description: 'An array of medal objects.'}
    },
  },
  unlock: {
    require_session: true,
  ...
  getScores: {
    ...
    params: {
      id: {
        type: Number,
        extract_from:
            {object: 'scoreboard', alias: 'scoreboard', property: 'id'},
        required: true,
    ...
    returns: {
      scoreboard:
          {object: 'scoreboard', description: 'The #scoreboard being queried.'},
      scores: {
        array: {object: 'score'},
  ...
  Object.defineProperty(this, 'session', {
    set: function(session) {
      if (session && !session.constructor === Newgrounds.io.model.session)
        throw new Error(
            '\'session\' must be a \'Newgrounds.io.model.session\' instance.');
      _session = session;
    },
    get: function() {
      return _session;
...
Newgrounds.io.SessionLoader.prototype = {
  ...
  startSession: function(callback, context) {
    ...
    this.__ngio.callComponent('App.startSession', function(r) {
      ...
      if (callback && callback.constructor === Function)
        callback.call(context, loader.session);
    });
  },
  ...
  endSession: function(callback, context) {
    ...
    this.__ngio.callComponent('App.endSession', function(r) {
      ...
      ngio.session_id = null;
      var event = new Newgrounds.io.events.SessionEvent(
          Newgrounds.io.events.SessionEvent.SESSION_EXPIRED);
      loader.dispatchEvent(event);
      if (callback && callback.constructor === Function)
        callback.call(context, loader.session);
    });
          ...
          v = (w.Hex = {
            ...
            parse: function(a) {
              for (var c = a.length, e = [], j = 0; j < c; j += 2)
                e[j >>> 3] |= parseInt(a.substr(j, 2), 16)
                    << (24 - 4 * (j % 8));
...
r = r.MD5 = v.extend({
  ...
  _doProcessBlock: function(q, n) {
    ...
    var a = this._hash.words, c = q[n + 0], e = q[n + 1], j = q[n + 2],
        k = q[n + 3], z = q[n + 4], r = q[n + 5], t = q[n + 6], w = q[n + 7],
        v = q[n + 8], A = q[n + 9], B = q[n + 10], C = q[n + 11], u = q[n + 12],
        D = q[n + 13], E = q[n + 14], x = q[n + 15], f = a[0], m = a[1],
        g = a[2], h = a[3], f = p(f, m, g, h, c, 7, b[0]),
        h = p(h, f, m, g, e, 12, b[1]), g = p(g, h, f, m, j, 17, b[2]),
        m = p(m, g, h, f, k, 22, b[3]), f = p(f, m, g, h, z, 7, b[4]),
        h = p(h, f, m, g, r, 12, b[5]), g = p(g, h, f, m, t, 17, b[6]),
        m = p(m, g, h, f, w, 22, b[7]), f = p(f, m, g, h, v, 7, b[8]),
        h = p(h, f, m, g, A, 12, b[9]), g = p(g, h, f, m, B, 17, b[10]),
        m = p(m, g, h, f, C, 22, b[11]), f = p(f, m, g, h, u, 7, b[12]),
        h = p(h, f, m, g, D, 12, b[13]), g = p(g, h, f, m, E, 17, b[14]),
        ...
        h = d(h, f, m, g, j, 9, b[29]), g = d(g, h, f, m, w, 14, b[30]),
        m = d(m, g, h, f, u, 20, b[31]), f = l(f, m, g, h, r, 4, b[32]),
        h = l(h, f, m, g, v, 11, b[33]), g = l(g, h, f, m, C, 16, b[34]),
        m = l(m, g, h, f, E, 23, b[35]), f = l(f, m, g, h, e, 4, b[36]),
        h = l(h, f, m, g, z, 11, b[37]), g = l(g, h, f, m, w, 16, b[38]),
        m = l(m, g, h, f, B, 23, b[39]), f = l(f, m, g, h, D, 4, b[40]),
        h = l(h, f, m, g, c, 11, b[41]), g = l(g, h, f, m, k, 16, b[42]),
        m = l(m, g, h, f, t, 23, b[43]), f = l(f, m, g, h, A, 4, b[44]),
        h = l(h, f, m, g, u, 11, b[45]), g = l(g, h, f, m, x, 16, b[46]),
        m = l(m, g, h, f, j, 23, b[47]), f = s(f, m, g, h, c, 6, b[48]),
        h = s(h, f, m, g, w, 10, b[49]), g = s(g, h, f, m, E, 15, b[50]),
        m = s(m, g, h, f, r, 21, b[51]), f = s(f, m, g, h, u, 6, b[52]),
        h = s(h, f, m, g, k, 10, b[53]), g = s(g, h, f, m, B, 15, b[54]),
        m = s(m, g, h, f, e, 21, b[55]), f = s(f, m, g, h, v, 6, b[56]),
        h = s(h, f, m, g, x, 10, b[57]), g = s(g, h, f, m, t, 15, b[58]),
        m = s(m, g, h, f, D, 21, b[59]), f = s(f, m, g, h, z, 6, b[60]),
        h = s(h, f, m, g, C, 10, b[61]), g = s(g, h, f, m, j, 15, b[62]),
    ...
    s = (p.EvpKDF = d.extend({
      ...
      compute: function(d, r) {
        for (var p = this.cfg, s = p.hasher.create(), b = l.create(),
                 u = b.words, q = p.keySize, p = p.iterations;
             u.length < q;
    ...
    d = (d.AES = p.extend({
      ...
      _doCryptBlock: function(a, b, c, d, e, j, l, f) {
        for (var m = this._nRounds, g = a[b] ^ c[0], h = a[b + 1] ^ c[1],
                 k = a[b + 2] ^ c[2], n = a[b + 3] ^ c[3], p = 4, r = 1;
             r < m; r++)
          var q = d[g >>> 24] ^ e[(h >>> 16) & 255] ^ j[(k >>> 8) & 255] ^
              l[n & 255] ^ c[p++],
              s = d[h >>> 24] ^ e[(k >>> 16) & 255] ^ j[(n >>> 8) & 255] ^
              l[g & 255] ^ c[p++],
              t = d[k >>> 24] ^ e[(n >>> 16) & 255] ^ j[(g >>> 8) & 255] ^
              l[h & 255] ^ c[p++],
              n = d[n >>> 24] ^ e[(g >>> 16) & 255] ^ j[(h >>> 8) & 255] ^
              l[k & 255] ^ c[p++],
              g = q, h = s, k = t;
